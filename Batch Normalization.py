...
def main():
    ...
    # trainset=datasets.MNIST(root='./number_mnist',train=True,transform=data_form)
    # testset=datasets.MNIST(root='./number_mnist',train=False,transform=data_form)
    ...
    net = nn.Sequential(
        ...
        nn.AvgPool2d(kernel_size=2,stride=2),
        nn.Flatten(), nn.Linear(16*5*5, 120),nn.BatchNorm1d(120),nn.ReLU(),
        nn.Linear(120,84),nn.BatchNorm1d(84),nn.ReLU(),
        nn.Linear(84,10)).to('cuda')
    def init_weight(m):
